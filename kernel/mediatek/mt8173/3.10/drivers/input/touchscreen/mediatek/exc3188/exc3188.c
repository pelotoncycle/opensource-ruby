...
#define RELEASE_DATE "2016/07/13"
...
#define EGALAX_DBG(level, fmt, args...)  do{ if(level&DbgLevel) \
					printk( KERN_DEFAULT"[egalax_i2c]: " fmt, ## args); }while(0)
...
static struct input_dev * allocate_Input_Dev(void)
{
	...
	input_set_capability(pInputDev, EV_KEY, KEY_WAKEUP);
...
static int egalax_i2c_measure(struct _egalax_i2c *egalax_i2c)
{
	...
	//EGALAX_DBG(DBG_INT, "[Touch][eGalax] egalax_i2c_measure entry\n");
...
static irqreturn_t egalax_i2c_interrupt(int irq, void *dev_id)
{
	...
	if (egalax_i2c->suspend && input_dev) {
		p_egalax_i2c_dev->suspend = 0;
...
{
	p_egalax_i2c_dev->suspend = 1;
	if (device_may_wakeup(&p_egalax_i2c_dev->client->dev)) {
...
static int __devinit egalax_i2c_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	{
		p_egalax_i2c_dev->interrupt_gpio = of_get_named_gpio(devnode, "int-gpio", 0);
	}
#else
	{
	...
	if( !gpio_is_valid(p_egalax_i2c_dev->interrupt_gpio) )//return true if valid
	{
		ret = -ENODEV;
		goto fail1;
	}
	...
	if (ret != 0) {
		printk("%s, device init wakeup error\n", __func__);
	}
...
static int egalax_i2c_ts_init(void)
{
	...
	int tmp;
	struct device_node *devnode;
	printk("[Touch][egalax] Driver init entry!\n");
	...
	upmu_set_rg_vgp4_sw_en(0x1);
	mdelay(10);

	/* to find compatible node */
	devnode = of_find_compatible_node(NULL, NULL, "eeti,exc3188");
	if (!devnode) {
		printk("[Touch][egalax] gpio_en_pin:can't find deivce tree node!\n");
		result = -1;
		goto fail;
	}

	tmp = of_get_named_gpio(devnode, "gpio_en_pin", 0);
	printk("[Touch][egalax] gpio_en_pin = %d\n",tmp);

	/* set CTP_EN pin level high */
	/* move it to bootable right after V5V pull up*/
