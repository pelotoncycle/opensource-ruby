...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	io_reg = devm_regulator_get(&client->dev, "reg-vgp4");
	//ret = regulator_set_voltage(io_reg, 1800000, 1800000);
	ret = regulator_set_voltage(io_reg, 1800000, 1800000);
	if (ret != 0)
		TPD_DMESG("Failed to set reg-rgp4 voltage: %d\n", ret);
#if 0
	GTP_ERROR("reg-vgp4 test\n");
	for(i = 0; i < 5; ++i)
	{
	GTP_ERROR("reg-vgp4 test on\n");
	ret = regulator_enable(io_reg);
	msleep(2);
	GTP_ERROR("reg-vgp4 test off\n");
