...
#define AX_MEDIUM_STATUS_MODE = 0x1a
...
static int ax88772_link_reset(struct usbnet *dev)
{
	u16 mode;
	struct ethtool_cmd ecmd = { .cmd = ETHTOOL_GSET };
	printk("Hiro ax88772_link_reset\n");
...
static int ax88772_stop(struct usbnet *dev)
{
	int ret;
	u16 *medium;
	printk("Hiro ax88772_stop \n");

	medium = kmalloc(2, GFP_ATOMIC);
	if (medium) {
		ret = asix_read_cmd(dev, AX_CMD_READ_MEDIUM_STATUS, 0, 0, 2, medium);
...
static const struct driver_info ax88772b_info = {
	...
	.flags = FLAG_ETHER | FLAG_FRAMING_AX | FLAG_LINK_INTR |
	         FLAG_MULTI_PACKET | FLAG_AVOID_UNLINK_URBS,
	.rx_fixup = asix_rx_fixup_common,
	.tx_fixup = asix_tx_fixup,
