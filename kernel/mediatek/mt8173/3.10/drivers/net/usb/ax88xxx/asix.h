...
#define D(x...) if (is_debug) printk("HiroETH " x)

/* Fail to get ip from DHCP after suspend/resume.
 *
 * Mantis issue id: 7245, 7545
 * 
 * For the issue, the mac address was erased at suspend stage, 
 * so we should set MAC address again when system resumed.
*/
#define WORKAROUND_FOR_GET_IP_FAIL	

/* Fail to get ip from DHCP when power on the deivce WITHOUT ethernet cable and then suspend.
 * change the default power saving mode (PSC) from 0x0800 --> 0x0900 
 *
 * If didn't change the default PSC, that can't get IP from DHCP daemon in the case power on the device
 * WITHOUT ethernet cable.   
 *
*/
#define ETHERNET_PHY_PS_CONFIG 0x0900
