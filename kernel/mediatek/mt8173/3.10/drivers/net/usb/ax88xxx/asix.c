...
#ifdef WORKAROUND_FOR_GET_IP_FAIL
...
static u8 MAC_DERIVES_FROM_MTKNVRAM = 0;
...
static void get_mac_address(struct usbnet *dev)
{
	int ret = 0;
	...
	ret = ax8817x_read_cmd(dev, AX88772_CMD_READ_NODE_ID,
				   0, 0, ETH_ALEN, buf);
	if (ret < 0) {
		D("%s() ================== Failed to read MAC address: %d\n", __func__, ret);
		deverr(dev, "Failed to read MAC address: %d", ret);
	}
	
	D("%s() ================== get MAC address: %x:%x:%x:%x:%x:%x (<Buf> %x:%x:%x:%x:%x:%x)\n", __func__,
	*(dev->net->dev_addr + 0),
	*(dev->net->dev_addr + 1),
	*(dev->net->dev_addr + 2),
	*(dev->net->dev_addr + 3),
	...
	*(buf + 5));
	...
	devwarn(dev, "get MAC address: %x:%x:%x:%x:%x:%x (<Buf> %x:%x:%x:%x:%x:%x)\n", 
	*(dev->net->dev_addr + 0),
	*(dev->net->dev_addr + 1),
	*(dev->net->dev_addr + 2),
	*(dev->net->dev_addr + 3),
...
static void asix_reset(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "asix");
	if (node) {
		gpio = of_get_gpio(node, 0);
		printk("%s(): gpio = %d\n", __func__, gpio);

		retval = gpio_request(gpio, "ethernet en pin");
		if (retval)
			printk("%s(): ethernet en pin, failue of setting\n", __func__);
...
static int ax8817x_check_ether_addr(struct usbnet *dev)
{
	...
	u8 * ptr;
	u8 hw_tmpbuf[ETH_ALEN];
	int hw_addr[ETH_ALEN];
	unsigned int result, sum;	
	MAC_DERIVES_FROM_MTKNVRAM = 1;

	/* Hiro add variables result and sum for wrong MAC address issue. */
	// MTK nvram get: just for MTK bootloader 
	result = 0; 	//for FF case 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
	sum = 0;	//for zero case 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	memset(hw_addr,0,sizeof(hw_addr));
	memset(hw_tmpbuf,0,sizeof(hw_tmpbuf));
	if(MAC_DERIVES_FROM_MTKNVRAM){
		ptr = strstr(saved_command_line, "hw_addr0=");
		ptr += strlen("hw_addr0=");
		hw_addr[0] = simple_strtol(ptr, NULL, 16);
		sum += hw_addr[0];
		if(hw_addr[0]==255)
			result += 1;
	
		ptr = strstr(saved_command_line, "hw_addr1=");
		ptr += strlen("hw_addr1=");
		hw_addr[1] = simple_strtol(ptr, NULL, 16);
		sum += hw_addr[1];
		if(hw_addr[1]==255)
			result += 1;

		ptr = strstr(saved_command_line, "hw_addr2=");
		ptr += strlen("hw_addr2=");
		hw_addr[2] = simple_strtol(ptr, NULL, 16);
		sum += hw_addr[2];
		if(hw_addr[2]==255)
			result += 1;
		
		ptr = strstr(saved_command_line, "hw_addr3=");
		ptr += strlen("hw_addr3=");
		hw_addr[3] = simple_strtol(ptr, NULL, 16);
		sum += hw_addr[3];
		if(hw_addr[3]==255)
			result += 1;

		ptr = strstr(saved_command_line, "hw_addr4=");
		ptr += strlen("hw_addr4=");
		hw_addr[4] = simple_strtol(ptr, NULL, 16);
		sum += hw_addr[4];
		if(hw_addr[4]==255)
			result += 1;
				
		ptr = strstr(saved_command_line, "hw_addr5=");
		ptr += strlen("hw_addr5=");
		hw_addr[5] = simple_strtol(ptr, NULL, 16);
		sum += hw_addr[5];
		if(hw_addr[5]==255)
			result += 1;
	
		if(result<6 && sum!=0){
			hw_tmpbuf[0] = (u8)hw_addr[0];
			hw_tmpbuf[1] = (u8)hw_addr[1];
			hw_tmpbuf[2] = (u8)hw_addr[2];
			hw_tmpbuf[3] = (u8)hw_addr[3];
			hw_tmpbuf[4] = (u8)hw_addr[4];
			hw_tmpbuf[5] = (u8)hw_addr[5];
			printk("mac_address_provider: %02X-%02X-%02X-%02X-%02X-%02X\n", hw_tmpbuf[0], hw_tmpbuf[1], hw_tmpbuf[2], hw_tmpbuf[3], hw_tmpbuf[4], hw_tmpbuf[5]);
			memcpy(tmp, hw_tmpbuf, ETH_ALEN);
		}
	}
	/* Hiro add variables result and sum for wrong mac issue. */
	...
	if (((*((u8*)tmp) == 0) && (*((u8*)tmp + 1) == 0) && (*((u8*)tmp + 2) == 0)) ||
		...
		printk("Found invalid NVRAM MAC address value ");
...
static int ax88772b_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	le16_to_cpus(tmp16);
		...
		/* change the default power saving mode (PSC) from 0x0800 --> 0x0900 */
		*tmp16 = *tmp16|ETHERNET_PHY_PS_CONFIG;
	#endif
	
	ax772b_data->psc = *tmp16 & 0xFF00;
	D("%s() ================== tmp16=0x%x, psc=0x%x\n", __func__, *tmp16, ax772b_data->psc);
	/* End of get EEPROM data */
...
static void ax88772b_link_reset(struct work_struct *work)
{
	...
	{
		u16 tmp16;
		#ifndef WORKAROUND_FOR_GET_IP_FAIL
		/* Hiro: We don't need to restore state of presvd_phy_advertise & presvd_phy_bmcr. */
		if (!ax772b_data->presvd_phy_advertise && !ax772b_data->presvd_phy_bmcr) {
			D("%s() ================== entry event=%d no_presvd_bmcr \n", __func__, PHY_POWER_UP);
			/* Preserve BMCR for restoring */
		...
		#ifndef WORKAROUND_FOR_GET_IP_FAIL
		/* Hiro: We don't need to restore state of presvd_phy_advertise & presvd_phy_bmcr. */
		D("%s() ================== entry event=%d Restore bmcr \n", __func__, PHY_POWER_UP);
		/* Restore Advertisement control reg */
		ax8817x_mdio_write_le(dev->net, dev->mii.phy_id, MII_ADVERTISE,
				      ax772b_data->presvd_phy_advertise);
		/* Restore BMCR */
