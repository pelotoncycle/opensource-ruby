...
//#define RESETWORKROUND
#ifdef RESETWORKROUND
	static struct wake_lock ax88xxx_rst_wakelock;
#endif
...
int axusbnet_open(struct net_device *net)
{
	...
	D("%s() ================== entry\n", __func__);
	D("%s() ================== netif_msg_ifup()=%d \n", __func__, netif_msg_ifup(dev));
...
static void kevent(struct work_struct *work)
{
	...
	D("%s() ================== entry dev->flags = 0x%lx\n", __func__, dev->flags);
	/* usb_clear_halt() needs a thread context */
	if (test_bit(EVENT_TX_HALT, &dev->flags)) {
		D("%s() ================== entry EVENT_TX_HALT\n", __func__);
		...
		D("%s() ================== exit EVENT_TX_HALT\n", __func__);
	}
	if (test_bit(EVENT_RX_HALT, &dev->flags)) {
		D("%s() ================== entry EVENT_RX_HALT\n", __func__);
		unlink_urbs(dev, &dev->rxq);
		...
		D("%s() ================== exit EVENT_RX_HALT\n", __func__);
	}

	/* tasklet could resubmit itself forever if memory is tight */
	if (test_bit(EVENT_RX_MEMORY, &dev->flags)) {
		struct urb	*urb = NULL;
		D("%s() ================== entry EVENT_RX_MEMORY\n", __func__);
		if (netif_running(dev->net))
	...
	if (test_bit(EVENT_LINK_RESET, &dev->flags)) {
		...
		D("%s() ================== exit EVENT_LINK_RESET\n", __func__);
	}

	if (dev->flags)
		devdbg(dev, "kevent done, flags = 0x%lx", dev->flags);

	D("%s() ================== exit dev->flags = 0x%lx\n", __func__, dev->flags);
}
...
void axusbnet_disconnect(struct usb_interface *intf)
{
	...
	/* system suspend disconnect ethernet will be rebooted-reference usbnet.c */ 
...
#endif
#ifdef RESETWORKROUND
	wake_lock_destroy(&ax88xxx_rst_wakelock);
#endif
...
axusbnet_probe(struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	#ifdef RESETWORKROUND
	wake_lock_init(&ax88xxx_rst_wakelock, WAKE_LOCK_SUSPEND, "ax88xxx");
	#endif
...
static void asix_reset(void)
{
	...
	int	retval = 0;
	D("%s() ================== entry\n", __func__);
	wake_lock_timeout(&ax88xxx_rst_wakelock, 10*HZ);
