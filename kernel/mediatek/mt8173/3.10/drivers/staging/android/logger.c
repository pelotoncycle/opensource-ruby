...
static void logger_kernel_write(const struct iovec *iov, unsigned long nr_segs)
{
	...
	struct logger_log *log_metrics = NULL;
	struct logger_entry header;
	struct timespec now;
	...
	list_for_each_entry(log, &log_list, logs)
	if (strncmp(log->misc.name, LOGGER_LOG_METRICS,
			strlen(LOGGER_LOG_METRICS)) == 0)
			log_metrics = log;

	mutex_lock(&log_metrics->mutex);
	 ...
	 * entries that encroach on readable buffer.
	 */
	fix_up_readers(log_metrics, sizeof(struct logger_entry) + header.len);

	do_write_log(log_metrics, &header, sizeof(struct logger_entry));
	...
	while (nr_segs-- > 0) {
		...
		/* write out this segment's payload */
		do_write_log(log_metrics, iov->iov_base, len);

		iov++;
		total_len += len;
	}

	mutex_unlock(&log_metrics->mutex);

	/* wake up any blocked readers */
	wake_up_interruptible(&log_metrics->wq);
}
