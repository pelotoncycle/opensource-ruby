...
int wait_sdio_autok_ready(void *data)
{
	...
	int id;
	unsigned int vcore_uv = 0;
#if !defined(UT_TEST) && defined(CONFIG_SDIOAUTOK_SUPPORT) && defined(_DVFS_ENABLE_)
	int is_screen_off;
#endif

	btmod = get_boot_mode();
	atomic_set(&autok_is_abort, 0);
#ifdef VCOREFS_IS_95M_SEGMENT
	if ((vcorefs_is_95m_segment()) && (btmod == META_BOOT)){
		/* Never run here */
		printk(KERN_ERR "[%s] Exit since META_BOOT\n", __func__);
		return 0;
	}
#else
	if (btmod == META_BOOT) {
	...
	if (1 /*(btmod!=META_BOOT) && (btmod!=FACTORY_BOOT) && (btmod!=ATE_FACTORY_BOOT) */) {
		...
		/* DVFS Define 1.0v as LV */
		if (is_screen_off)
			autok_add_suggest_vol(1000000);
#endif
							...
							const char *buf, size_t count)
{
	...
	ret = sscanf(attr->attr.name, "%s", cur_name);
	for (i = 0; i < TOTAL_HOST_NODE_COUNT; i++) {
		test_len = strlen(host_nodes[i]);
		cur_len = strlen(cur_name);
		if ((test_len == cur_len) && (strncmp(host_nodes[i], cur_name, cur_len) == 0)) {
		...
		if (is_sug_more_than_real(id)) {
			for (i = 0; i < HOST_MAX_NUM; i++) {
				if (p_autok_thread_data->p_autok_progress[i].host_id == id) {
					/*
					p_autok_thread_data->p_autok_progress[i].done = 0;
					task2 = kthread_run(&trigger_1_0_autok_func,
										(void *)&id, "autok_1_0_task"); */
...
*/
	pr_err("[AUTO_K][%s] 2. Wifi 1.0v Autok Simulation\n", __func__);
	return 0;
}
#endif
