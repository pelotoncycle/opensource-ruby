...
static void msdc_init_clk(struct platform_device *pdev, struct msdc_host *host)
{
	...
	strncpy(clk_name, "peri_msdc0", 11);
	clk_name[9] += host->id;
#ifndef CFG_DEV_MSDC2
	if (host->id == 1) {
		clk = devm_clk_get(&pdev->dev, "peri_msdc2");
		BUG_ON(IS_ERR(clk));
		clk_prepare(clk);
		clk_enable(clk);
		clk_disable(clk);
	...
	pr_err("[MSDC%d] [%s] clk_name = %s, host->clk=0x%p\n",
		host->id, __func__, clk_name, host->clk);
	...
	if (host->id == 0) {
		...
		clk = devm_clk_get(&pdev->dev, "top_msdc50_0");
		pr_err("[MSDC%d] [%s] clk_name = %s, clk=0x%p\n",
			host->id, __func__, "top_msdc50_0", clk);
		BUG_ON(IS_ERR(clk));

		if (host->hw->clk_src == MSDC_CLKSRC_400MHZ)
			clk_parent = devm_clk_get(&pdev->dev, "top_msdcpll_d2");
		else if (host->hw->clk_src == MSDC_CLKSRC_800MHZ)
			clk_parent = devm_clk_get(&pdev->dev, "top_msdcpll_ck");
		else
			clk_parent = devm_clk_get(&pdev->dev, "top_msdcpll_d4");

		BUG_ON(IS_ERR(clk_parent));

		clk_prepare_enable(clk);
		if (IS_ERR_VALUE(clk_set_parent(clk, clk_parent))) {
			pr_err("[MSDC%d] [%s] clk_name = %s, set clk parent ERROR!\n",
				host->id, __func__, "top_msdc50_0");
		}
		clk_disable_unprepare(clk);
	#endif

		/* msdcpll2 not used, disable it for low power */
		clk = devm_clk_get(&pdev->dev, "msdcpll2");
		pr_err("[MSDC%d] [%s] clk_name = %s, host->clk=0x%p\n",
			host->id, __func__, "msdcpll2", clk);
		BUG_ON(IS_ERR(clk));
		clk_prepare(clk);
		clk_enable(clk);
		clk_disable(clk);
