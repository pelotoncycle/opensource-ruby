		...
		P_AUTOK_RD_TUNE_RES_T pRDTuneRes)
{
 ...
 ReTuneCMDPad:
	...
	if (stg == TUNING_STG2) {
		/* Usually, It should be have 1 fail region only */
		if (CMDPadScan.Reg1Cnt && (!CMDPadScan.BoundReg2_S) && CMDPadScanEdge) {
			CMDDlySel = CMDPadScan.BoundReg1_S +
				(CMDPadScan.BoundReg1_E - CMDPadScan.BoundReg1_S) / 2;
		} else if (CMDPadScan.Reg1Cnt && (!CMDPadScan.BoundReg2_S) && !CMDPadScanEdge) {
			...
			if (CMDIntPassWin >= 32) {
				...
				tmp = (int)31 - (int)CMDPadScan.BoundReg1_E - (int)CMDPadScan.BoundReg1_S;
				sel = (tmp <= AUTOK_TUNING_INACCURACY) ? 0 : 31;
		...
		/* The scan can NOT cover the boundary */
			...
			((CMDDlyScanE - PadDlyCMDRef) < CMDScanRangeStg2))) {
			AUTOK_PRINT(
				"[WARN] Can NOT find the suitable CMD pad delay!"
				"the temp shift is too much!!!\r\n");
			CMDDlySel = PadDlyCMDRef;
			rCmdTuneRes.fTimingShiftLarge = 1;
		} else {
			AUTOK_PRINT("[ERR] Undefined scenario when scan CMD pad delay!!!\r\n");
		...
		/* In case of Right boundary at CKGEN = 0 */
			...
			if (FBoundCnt && (FBoundCKGEN != SCALE_CKGEN_MSDC_DLY_SEL)) {
				...
				RBoundPad =
					...
					MIN_PAD_DELAY_IN_PS) + LBoundPad - FBoundCnt;
...
static void autok_vcore_set(unsigned int vcore_uv)
{
...
#if !defined(AUTOK_VCORE_SET_LEGACY) && defined(_DVFS_ENABLE_) && defined(CONFIG_SDIOAUTOK_SUPPORT)
	int ret;
#endif
	size = sizeof(mt65x2_vcore_tbl) / sizeof(mt65x2_vcore_tbl[0]);
	...
	pmic_config_interface(0x278, vcore_sel, 0xFFFF, 0);
	pmic_config_interface(0x27A, vcore_sel, 0xFFFF, 0);
#else
#ifdef CONFIG_SDIOAUTOK_SUPPORT
