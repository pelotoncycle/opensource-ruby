...
int ext_disp_init(struct platform_device *dev, char *lcm_name, unsigned int session)
{
	...
	if (!hdmi_config_rdma_task && !boot_up_with_facotry_mode() && ext_disp_use_frc && ext_disp_mode != EXTD_DIRECT_LINK_MODE) {
...
#if 0
	/* wait frame done before destory path */
...
int ext_disp_suspend(void)
{
	...
	if (!(DISP_REG_GET(DISP_REG_CONFIG_MMSYS_HW_DCM_DIS0) & 0x00001000) && !ext_disp_is_decouple_mode()) {
...
#if HDMI_SUB_PATH
	DISPMSG("%s ext disp is sleeped %d is_hdmi_active %d\n", __func__,
