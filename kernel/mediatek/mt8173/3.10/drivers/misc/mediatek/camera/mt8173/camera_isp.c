...
struct clk *g_ispclk_top_mm_sel;
...
static inline void Get_ccf_clock(struct platform_device *pDev)
{
	...
	g_ispclk_top_mm_sel = devm_clk_get(&pDev->dev, "TOP_MM_SEL");
	BUG_ON(IS_ERR(g_ispclk_top_mm_sel));
...
{
	BUG_ON(IS_ERR(g_ispclk_top_mm_sel));
	clk_prepare(g_ispclk_top_mm_sel);
	BUG_ON(IS_ERR(g_ispclk_scp_sys_isp));
...
{
	BUG_ON(IS_ERR(g_ispclk_top_mm_sel));
	clk_enable(g_ispclk_top_mm_sel);
	BUG_ON(IS_ERR(g_ispclk_scp_sys_isp));
...
static inline void Disable_ccf_clock(void)
{
	...
	BUG_ON(IS_ERR(g_ispclk_top_mm_sel));
	clk_disable(g_ispclk_top_mm_sel);
	return;
}
...
static inline void Unprepare_ccf_clock(void)
{
	...
	BUG_ON(IS_ERR(g_ispclk_top_mm_sel));
	clk_unprepare(g_ispclk_top_mm_sel);
	return;
}
