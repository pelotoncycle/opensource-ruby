...
static void accdet_eint_work_callback(struct work_struct *work)
{
	...
	disable_irq(irq_num);
	ACCDET_DEBUG("[Accdet]disable irq\n");
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		ACCDET_DEBUG("[Accdet]EINT func :headset plug-in\n");
		mutex_lock(&accdet_eint_irq_sync_mutex);
...
/* EINT_PIN_PLUG_OUT */
/* Disable ACCDET */
		ACCDET_DEBUG("[Accdet]EINT func :headset plug-out\n");
		mutex_lock(&accdet_eint_irq_sync_mutex);
	...
	/* msleep(500); */
	enable_irq(irq_num);
	ACCDET_DEBUG("[Accdet]enable irq\n");
