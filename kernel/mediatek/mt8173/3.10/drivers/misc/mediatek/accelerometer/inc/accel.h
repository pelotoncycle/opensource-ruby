...
#define ACC_ERR(fmt, args...)        pr_err(KERN_ERR ACC_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
#define ACC_LOG(fmt, args...)        pr_debug(KERN_ERR ACC_TAG fmt, ##args)
#define ACC_VER(fmt, args...)       pr_debug(KERN_ERR ACC_TAG"%s: "fmt, __func__, ##args)
...
struct acc_drv_obj {
    void *self;
    int polling;
    int (*acc_operate)(void *self, uint32_t command, void *buff_in, int size_in, void *buff_out, int size_out, int *actualout);
};
