...
void vShowHpdRsenStatus(void)
{
	...
	if (hdcp2_version_flag == TRUE)
		printk("[HDMI]support hdcp2.2\n");
	else
		printk("[HDMI]support hdcp1.x\n");

	printk("[HDMI]hdmi2_force_output=%d\n", hdmi2_force_output);
...
void MuteHDMI2Audio(void)
{
	...
	vWriteByteHDMIUnMsk(HDCP_TOP_CTRL,OTPADROPOVR_SET);
