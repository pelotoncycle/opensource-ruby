...
#define MTKFB_LOG(fmt, args...) \
		do { \
		if (mtkfb_log_on) pr_err("[MTKFB]"fmt, ##args); \
		} while (0)

#define MTKFB_FUNC()	\
	do { \
		if (mtkfb_log_on) pr_err("[MTKFB Func]%s %d\n", __func__, __LINE__); \
	} while (0)
...
void mtkfb_dump_layer_info(void)
{
#if 0
	unsigned int i;
	MTKFB_MSG("[mtkfb] start dump layer info, early_suspend=%d\n", primary_display_is_sleepd());
	MTKFB_MSG("[mtkfb] cache(next):\n");
	for (i = 0; i < 4; i++) {
		MTKFB_MSG("[mtkfb] layer=%d, layer_en=%d, idx=%d, fmt=%d, addr=0x%x, %d, %d, %d\n ", cached_layer_config[i].layer,	/* layer */
	...
	MTKFB_MSG("[mtkfb] captured(current):\n");
	for (i = 0; i < 4; i++) {
	...
	case MTKFB_PREPARE_OVERLAY_BUFFER:
		{
			...
			if (copy_from_user
				...
				MTKFB_ERR("[FB Driver]: copy_from_user failed! line:%d\n", __LINE__);
				r = -EFAULT;
			} else {
				...
				if (copy_to_user
					...
					MTKFB_ERR("[FB Driver]: copy_to_user failed! line:%d\n",
					       __LINE__);
					return -EFAULT;
				}
			}
...
int __init mtkfb_init(void)
{
	...
	MTKFB_MSG("mediatek frame buffer driver init\n");
