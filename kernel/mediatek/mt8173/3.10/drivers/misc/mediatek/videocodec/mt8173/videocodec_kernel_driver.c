...
static struct clk *clk_venc_lt_larb, *clk_venc_lt_clk, *clk_vdecpwr, *clk_venc_pwr, *clk_venc_pwr2, *clk_mm_sel, *clk_mm_mtcmos, *clk_mm_vdec;
#endif
...
void vdec_power_on(void)
{
	...
	clk_prepare(clk_mm_vdec);
	clk_enable(clk_mm_vdec);
	clk_prepare(clk_mm_mtcmos);
	clk_enable(clk_mm_mtcmos);
...
void vdec_power_off(void)
{
	...
	if (gu4VdecPWRCounter == 0) {
		...
		clk_disable(clk_mm_mtcmos);
		clk_unprepare(clk_mm_mtcmos);
		clk_disable(clk_mm_vdec);
		clk_unprepare(clk_mm_vdec);
...
static int vcodec_probe(struct platform_device *pdev)
{
	...
	clk_mm_sel = devm_clk_get(&pdev->dev, "MT_TOP_MM_SEL");
	BUG_ON(IS_ERR(clk_mm_sel));
	clk_mm_mtcmos = devm_clk_get(&pdev->dev, "MMSYS_CLK_MTCMOS");
	BUG_ON(IS_ERR(clk_mm_mtcmos));
	clk_mm_vdec = devm_clk_get(&pdev->dev, "MT_TOP_MM_VDEC");
	BUG_ON(IS_ERR(clk_mm_vdec));
