...
static unsigned int accdet_eint[2] = {0};
static unsigned int accdet_debounce = 0;
...
int board_sdio_ctrl(unsigned int sdio_port_num, unsigned int on)
{
	...
	if (!combo_port_pwr_map[sdio_port_num] && on) {
		...
		if (wifi_irq != 0xfffffff)
			...
			pr_warn("wifi_irq is not available\n");
...
static struct headset_mode_settings cust_headset_settings = {
	0x900, 0x400, 1, 0x3f0, 0x9000, 0x800, 0x20
};
#else
...
static int __init cust_headset_data_init(void)
{
	...
	if (node) {
		ret = of_property_read_u32_array(node, "interrupts" , accdet_eint, ARRAY_SIZE(accdet_eint));
		if (ret) {
			pr_err("[%s] interrupts get fail, ret:%d\n", __func__, ret);
			return ret;
		}

		ret = of_property_read_u32(node, "debounce", &accdet_debounce);
		if (ret) {
			pr_err("[%s] debounce get fail, ret:%d\n", __func__, ret);
			return ret;
		}

		pr_info("[%s] cust_eint_accdet_num:%d, cust_eint_accdet_debounce_cn:%d, cust_eint_accdet_type:%d\n", __func__, accdet_eint[0], accdet_debounce, accdet_eint[1]);

		cust_headset_data.cust_eint_accdet_num = accdet_eint[0];
		cust_headset_data.cust_eint_accdet_debounce_cn = accdet_debounce;
		cust_headset_data.cust_eint_accdet_type = accdet_eint[1];
	}
	return ret;
}

fs_initcall(cust_headset_data_init);

static int board_version = BOARD_REV_INVALID;

BOARD_VER get_board_version(void)
{
	return board_version;
}

static int __init board_version_setup(char *str)
{
	get_option(&str, &board_version);

	pr_info("%s():board_version=%d\n", __func__, board_version);

	return 0;
}
__setup("androidboot.board.version=", board_version_setup);
