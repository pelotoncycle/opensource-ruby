...
unsigned int max_power_in_power_table = 0;
		...
		for (tlp = 1; tlp <= max_tlp; tlp++) {
			...
			for (i = 0; i < CA53_CA57_FREQ_COMBINATION; i++) {
				for (j = 0; j < FREQ_LEVEL_NUM; j++) {
					...
					if (curr_power > max_power_in_power_table)
						max_power_in_power_table = curr_power;
...
int mt_cpufreq_thermal_protect(unsigned int limited_power, unsigned int limitor_index)
{
	...
	for (i = 0; i < MAX_LIMITOR_NUM; i++) {
		...
		else if (all_limited_power[i] && (all_limited_power[i] < limited_power))
			limited_power = all_limited_power[i];
	}
	...
	/* no limited */
	if (0 == limited_power || limited_power > max_power_in_power_table) {
		for_each_cpu_dvfs(i, p) {
			if (!cpu_dvfs_is_availiable(p))
				continue;
			p->limited_max_ncpu =
...
static int _thermal_limited_verify(struct mt_cpu_dvfs *p, int new_opp_idx)
{
...
#if defined(CONFIG_CPU_FREQ_GOV_HOTPLUG)
	{
		...
		cpufreq_dbg("%s(): name=%s, begin, idx = %d, online_cpu = %d, xxx = %d\n", __func__,
			p->name, new_opp_idx, online_cpu, p->limited_freq_by_hevc);
	}
#else
	...
	/* no limited */
	if (0 == p->thermal_protect_limited_power || p->thermal_protect_limited_power > max_power_in_power_table)
		return new_opp_idx;
...
static int cpufreq_limited_power_proc_show(struct seq_file *m, void *v)
{
	...
	for_each_cpu_dvfs(i, p) {
		...
		seq_printf(m, "[%s/%d] %d @ %d\n"
			   "limited_max_freq = %d\n"
			   "limited_max_ncpu = %d\n",
			   p->name, i, p->thermal_protect_limited_power, max_power_in_power_table,
