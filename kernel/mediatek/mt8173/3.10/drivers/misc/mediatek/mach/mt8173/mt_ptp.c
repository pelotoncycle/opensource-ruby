...
static inline void ptp_isr_handler(struct ptp_det *det)
{
	...
	if (ptp_log_en) {
		...
		ptp_isr_info("[PTPOD]: PTP_AGEVALUES = 0x%x\n", ptp_read(PTP_AGEVALUES));
		ptp_isr_info("[PTPOD]: PTP_CHKSHIFT = 0x%x\n", ptp_read(PTP_CHKSHIFT));
		ptp_isr_info("[PTPOD]: TEMPMSR0  = 0x%x\n", ptp_read(TEMPMSR0));
		ptp_isr_info("[PTPOD]: TEMPMSR1 = 0x%x\n", ptp_read(TEMPMSR1));
		ptp_isr_info("[PTPOD]: TEMPMSR2 = 0x%x\n", ptp_read(TEMPMSR2));
		ptp_isr_info("[PTPOD]: TEMPMSR3 = 0x%x\n", ptp_read(TEMPMSR3));
	}
