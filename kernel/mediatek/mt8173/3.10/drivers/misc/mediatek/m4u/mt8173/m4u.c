...
static int m4u_probe(struct platform_device *pdev)
{
	...
	if (0 == pdev->id) {
		...
		if (!m4u_tee_en) {
			m4u_buf_info_t *pMvaInfo;
			unsigned int mva;
			unsigned int tee_mem_size = 0;

			m4u_session_init();
			m4u_sec_init(m4u_get_domain_by_id(0)->pgd_pa, 1, &tee_mem_size);

			pMvaInfo = m4u_alloc_buf_info();
			if (pMvaInfo) {
				pMvaInfo->port = M4U_PORT_UNKNOWN;
				pMvaInfo->size = tee_mem_size;
				mva = m4u_do_mva_alloc(0, tee_mem_size, pMvaInfo);
				pMvaInfo->mva = mva;
				M4UMSG("reserve sec mva: 0x%x\n", mva);
			}
		}
#endif
	}
