...
static void set_adaptive_cpu_power_limit(unsigned int limit)
{
	...
	if (prv_adp_cpu_pwr_lim != adaptive_cpu_power_limit) {
		tscpu_printk("set_adaptive_cpu_power_limit %d, T=%d,%d,%d,%d,%d,%d,%d,%d\n",
...
static void set_static_cpu_power_limit(unsigned int limit)
{
	...
	if (prv_stc_cpu_pwr_lim != static_cpu_power_limit) {
		tscpu_printk("set_static_cpu_power_limit %d, T=%d,%d,%d,%d,%d,%d,%d,%d\n",
			     (final_limit != 0x7FFFFFFF) ? final_limit : 0, GPU_TS1_T, CA7_TS2_T,
			     CA15_TS2_T, GPU_TS2_T, CORE_TS2_T, CA7_TS3_T, CA15_TS4_T, GPU_TSABB_T);
...
static void thermal_interrupt_handler(int bank)
{
	...
	if (ret & THERMAL_tri_SPM_State1) {
		...
		tscpu_printk("THERMAL_tri_SPM_State1, T=%d,%d,%d,%d,%d,%d,%d,%d\n",
...
/* THERMAL_WRAP_WR32(0x00010507, TEMPMONCTL2);        // filt interval is 1 * 46.540us = 46.54us, sen interval is 1287 * 46.540us = 59.89 ms */
	...
	THERMAL_WRAP_WR32((auxadc_ts_phy_base + 0x008), TEMPADCENADDR);	/* AHB address for auxadc enable (channel 0 immediate mode selected) */
...
static void tscpu_set_GPIO_toggle_for_monitor(void)
{
	...
	if (g_GPIO_out_enable == 1) {
		if (g_max_temp > g_trigger_temp) {
			...
			tscpu_printk
			    ("tscpu_set_GPIO_toggle_for_monitor:lv_GPIO8_MODE=0x%x,lv_GPIO8_DIR=0x%x,lv_GPIO8_DOUT=0x%x,\n",
			     lv_GPIO8_MODE, lv_GPIO8_DIR, lv_GPIO8_DOUT);
				    ...
				    loff_t *data)
{
	...
	if (sscanf(desc, "%s %d %s %d ", TEMP, &valTEMP, ENABLE, &valENABLE) == 4) {
		...
		thermal_clrl(GPIO8_MODE, 0x00000E00);	/* clear GPIO8_MODE[11:9],GPIO8_MODE = 0 (change to GPIO mode) */
...
static int get_io_reg_base(void)
{
	...
	if (node) {
		...
		/* printk("[THERM_CTRL] pericfg_base=0x%x\n",pericfg_base); */
	}
...
static int tscpu_thermal_probe(struct platform_device *pdev)
{
	...
	if (!mtktscpu_dir) {
		tscpu_printk("tscpu_thermal_probe mkdir /proc/driver/thermal failed\n");
	} else {
		...
		/* +ASC+ */
		entry =
		    proc_create("clatm", S_IRUGO | S_IWUSR, mtktscpu_dir,
				&mtktscpu_atm_fops);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
			proc_set_user(entry, 0, 1000);
