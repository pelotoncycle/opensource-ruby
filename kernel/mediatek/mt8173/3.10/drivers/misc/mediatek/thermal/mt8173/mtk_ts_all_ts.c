...
static int __init tsallts_init(void)
{
	...
	tsallts_dprintk("tsallts_register_cooler:err=%x\n", err);
	if (err)
		return err;
	...
	if (!tsallts_dir) {
		tsallts_dprintk("[tsallts_init]: mkdir /proc/driver/thermal failed\n");
	} else {
		entry =
		...
		entry =
		    proc_create("tzts2", S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops2);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
			proc_set_user(entry, 0, 1000);
#else
