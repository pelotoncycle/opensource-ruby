...
static void cmdq_core_attach_error_task(const struct TaskStruct *pTask, int32_t thread)
{
	...
	if (NULL != pTask) {
		CMDQ_ERR("=============== [CMDQ] Error Thread PC Task[%p] trigger[%lld]===============\n",
				pTask, pTask->trigger);
		hwPC = cmdq_core_dump_pc(pTask, thread, "ERR");

		CMDQ_ERR("=============== [CMDQ] Begin of Error Task[%p] trigger[%lld] Status ===============\n",
					pTask, pTask->trigger);
		cmdq_core_dump_task(pTask);
		cmdq_core_dump_instructions((uint64_t *) pTask->pVABase, pTask->commandSize);

		CMDQ_ERR("=============== [CMDQ] End of Error Task[%p] trigger[%lld] Status ===============\n",
					pTask, pTask->trigger);
	}
								...
								int32_t thread)
{
	...
	while (0 == waitQ && retryCount < CMDQ_PREDUMP_RETRY_COUNT) {
		CMDQ_LOG("==========[CMDQ] timeout Pre-dump(%d) pTask[0x%p], trigger[%lld]==========\n",
		...
		if (0 == retryCount) {
			...
			if (gCmdqContext.errorTask.errTaskCount < CMDQ_MAX_ERROR_COUNT) {
				...
				pErrorTask = &gCmdqContext.errorTask.errorTaskBuffer[gCmdqContext.errNum];
...
int cmdq_dev_enable_device_clock(bool enable, struct clk *clk_module, const char *clkName)
{
	int result = 0;

	if (IS_ERR(clk_module)) {
		CMDQ_ERR("CCF Clock:%s enable:%d Fail!\n", clkName, enable);
		result = -1;
		BUG_ON(1);
	}
...
{
	return cmdq_dev_enable_device_clock(enable, sys_dis, "MTCMOS_DIS-clk");
}
