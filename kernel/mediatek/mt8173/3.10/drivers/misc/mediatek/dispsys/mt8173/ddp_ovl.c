		     ...
		     unsigned int yuv_range, void *handle)
{
	...
	DISP_REG_SET(handle, DISP_REG_OVL_L0_ADDR + layer_off_site,
		     addr + src_pitch * (src_y + dst_h - 1) + (src_x + dst_w) * bpp - 1);
#else
	DISP_REG_SET(handle, DISP_REG_OVL_L0_ADDR + layer_off_site,
	...
	if (has_sec_layer) {
		...
		if (ovl_is_sec[ovl_idx] == 1) {
			...
			if (ovl_idx == 1 && is_hdmi_active() && !ext_disp_is_decouple_mode()) {
				ovl_cmdq_insert_wait_frame_done_token_mira(nonsec_switch_handle, ext_disp_get_mutex_id());
				mode = CMDQ_DISP_VIDEO_MODE;
			}
		#endif
			...
			for (i = 0; i < OVL_LAYER_NUM; i++) {
				...
				ovl_layer_switch(module, i, pConfig->ovl_config[i].layer_en, nonsec_switch_handle, 1);
			}
