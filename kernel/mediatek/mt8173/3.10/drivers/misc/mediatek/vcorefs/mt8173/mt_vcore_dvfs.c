...
static void __update_vcore(struct pwr_ctrl *pwrctrl, int steps)
{
	...
	regulator_set_voltage(reg_vcore_dvfs, vcore_curr_uv, vcore_curr_uv + 6250 - 1);
...
int set_faxi_fxxx_with_opp(struct pwr_ctrl *pwrctrl, struct dvfs_opp *opp)
{
...
#ifdef MTK_WCN_CMB_FOR_SDIO_1V_AUTOK
	r = mtk_wcn_cmb_stub_1vautok_for_dvfs();
	if (r)
...
int do_dvfs_for_performance(struct pwr_ctrl *pwrctrl, struct dvfs_opp *opp)
{
	int r;
...
int do_dvfs_for_low_power(struct pwr_ctrl *pwrctrl, struct dvfs_opp *opp)
{
	int r;
 ...
 *   < 0: failure
 */
int __kick_dvfs_by_index(struct pwr_ctrl *pwrctrl, u32 index)
{
	int err;
...
int kick_dvfs_by_index(struct pwr_ctrl *pwrctrl, enum dvfs_kicker kicker, u32 index)
{
	int err = 0;
	bool in_autok = false;
 ...
 **************************************/
void kick_dvfs_after_screen_off(struct pwr_ctrl *pwrctrl)
{
