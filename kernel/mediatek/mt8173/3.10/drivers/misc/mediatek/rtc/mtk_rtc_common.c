...
static struct delayed_work	ntp_store_work;

static void ntp_write_product_info_ntp_time(char * ntp_time)
{
	int ret, part_offset, size;
	struct file *fp = NULL;
	mm_segment_t fs;

	/* Backup current domain of FS */
	fs = get_fs();
	/* Set FS to kernel domain */
	set_fs(KERNEL_DS);

	/* /dev/block/platform/mtk-msdc.0/by-name/PRO_INFO */
	fp = filp_open("/dev/block/mmcblk0p1", O_WRONLY, 0);

	if (IS_ERR_OR_NULL(fp)) {
		pr_err("%s : failed to open device for writing\n", __func__);
		goto exit;
	}

	if (fp->f_op == NULL) {
		pr_err("%s : There is not operations for fp\n", __func__);
		goto exit;
	}

	if (fp->f_op->llseek == NULL) {
		pr_err("%s : There is not llseek operations for fp\n", __func__);
		goto exit;
	}

	if (fp->f_op->write == NULL) {
		pr_err("%s : There is not write operations for fp\n", __func__);
		goto exit;
	}

	size = strlen(ntp_time) + 1;
	part_offset = 512;
	ret = fp->f_op->llseek(fp, part_offset, SEEK_SET);

	if (ret != part_offset) {
		pr_err("%s : llseek operations failed, error=%d\n", __func__, ret);
		goto exit;
	}

	pr_err("%s : writing %d bytes\n", __func__, size);

	ret = fp->f_op->write(fp, ntp_time, size, &fp->f_pos);

	if (ret != size) {
		pr_err("%s : write operations failed, error=%d\n", __func__, ret);
		goto exit;
	}
...
exit:
	if (!IS_ERR_OR_NULL(fp)) {
		...
		if (ret) {
			pr_err("%s : filp_close() failed, error=%d\n", __func__, ret);
		}
	}

	/* Restore previous domain of FS */
	set_fs(fs);
}

static void ntp_store_rtc_time(void)
{
	struct rtc_time tm;
	char ntp_time[16];

	hal_rtc_get_tick_time(&tm);
	sprintf(ntp_time, "%04d%02d%02d-%02d%02d%02d",
			tm.tm_year + RTC_MIN_YEAR,
			tm.tm_mon,
			tm.tm_mday,
			tm.tm_hour,
	...
	pr_err("%s : storing(%s)\n", __func__, ntp_time);
	ntp_write_product_info_ntp_time(ntp_time);
}

static void ntp_store_worker(struct work_struct *work)
{
	ntp_store_rtc_time();
}
...
static int rtc_ops_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	cancel_delayed_work_sync(&ntp_store_work);
	schedule_delayed_work(&ntp_store_work, 0);
...
static int __init rtc_late_init(void)
{
	...
	INIT_DELAYED_WORK(&ntp_store_work, ntp_store_worker);
