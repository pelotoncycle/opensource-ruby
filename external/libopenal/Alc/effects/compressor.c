...
static ALvoid ALcompressorState_process(ALcompressorState *state, ALuint SamplesToDo, const ALfloat (*SamplesIn)[BUFFERSIZE], ALfloat (*SamplesOut)[BUFFERSIZE], ALuint NumChannels);
...
static ALvoid ALcompressorState_update(ALcompressorState *state, const ALCdevice *device, const ALeffectslot *slot, const ALeffectProps *props)
{
    ...
    //gain 0.01 ~ 1.0f , rate 0.000022676f ~ 1.0f
    state->GainCtrl			= props->Compressor.GainCtrl;
    state->AttackRate 		= props->Compressor.AttackRate;
    state->ReleaseRate 	= props->Compressor.ReleaseRate;

    STATIC_CAST(ALeffectState,state)->OutBuffer = device->FOAOut.Buffer;
    STATIC_CAST(ALeffectState,state)->OutChannels = device->FOAOut.NumChannels;
    for(i = 0;i < 4;i++)
        ComputeFirstOrderGains(device->FOAOut, IdentityMatrixf.m[i], slot->Params.Gain, state->Gain[i]);
}
    ...
    for(base = 0;base < SamplesToDo;)
    {
        ...
        if(state->Enabled)
        {
            ...
            for(i = 0;i < td;i++)
            {
                 ...
                 * the volume. */
                //output = 1.0f / clampf(gain, 0.5f, 2.0f); //by stone
                output = clampf(gain, 0.01f, 100.0f);
                for(j = 0;j < 4;j++)
                    temps[i][j] *= output;
            }
...
static ALeffectState *ALcompressorStateFactory_create(ALcompressorStateFactory *UNUSED(factory))
{
    ...
    state->Enabled 			= AL_TRUE;
    state->GainCtrl 			= 1.0f;
    state->AttackRate 		= 0.0001f;
    state->ReleaseRate 	= 0.0001f;
...
void ALcompressor_setParamf(ALeffect* effect, ALCcontext* context, ALenum param, ALfloat val)
{
	...
	switch(param)
	{
		case AL_COMPRESSOR_GAINCTRL:
			props->Compressor.GainCtrl = val;
			break;

		case AL_COMPRESSOR_ATTACKRATE:
			props->Compressor.AttackRate = val;
			break;

		case AL_COMPRESSOR_RELEASERATE:
			props->Compressor.ReleaseRate = val;
			break;
...
//by stone
void ALcompressor_getParamf(const ALeffect* effect , ALCcontext* context, ALenum param, ALfloat* val)
{
	const ALeffectProps *props = &effect->Props;
	switch(param)
	{
		case AL_COMPRESSOR_GAINCTRL:
			*val = props->Compressor.GainCtrl;
			break;

		case AL_COMPRESSOR_ATTACKRATE:
			*val = props->Compressor.AttackRate;
			break;

		case AL_COMPRESSOR_RELEASERATE:
			*val = props->Compressor.ReleaseRate;
			break;
