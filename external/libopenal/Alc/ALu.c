...
inline void aluMatrixfSetRow(aluMatrixf *matrix, ALuint row, ALfloat m0, ALfloat m1, ALfloat m2, ALfloat m3)
{
    matrix->m[row][0] = m0;
    matrix->m[row][1] = m1;
    matrix->m[row][2] = m2;
...
static ALfloat ApplyLimiter(ALfloat (*OutBuffer)[BUFFERSIZE], const ALsizei NumChans,
                            const ALfloat AttackRate, const ALfloat ReleaseRate,
                            const ALfloat InGain, ALfloat (*Gains),
...
{                                                                             \
    ...
    {                                                                         \
        ...
        ALfloat *distbuf = ASSUME_ALIGNED(distcomp[j].Buffer, 16);   \
        if(base > 0 || gain != 1.0f)                                          \
        {                                                                     \
            if(SamplesToDo >= base)                                           \
            {                                                                 \
