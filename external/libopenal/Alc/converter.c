...
static inline void Load_##T(ALfloat *dst, const T *src,     \
...
#define DECL_TEMPLATE(T)                                                      \
static inline void Store_##T(T *dst, const ALfloat *src,    \
...
ALsizei SampleConverterInput(SampleConverter *converter, const ALvoid **src, ALsizei *srcframes, ALvoid *dst, ALsizei dstframes)
{
    ...
    while(pos < dstframes)
    {
        ALfloat *SrcData = ASSUME_ALIGNED(converter->mSrcSamples, 16);
        ALfloat *DstData = ASSUME_ALIGNED(converter->mDstSamples, 16);
        ALint prepcount = converter->mSrcPrepCount;
        ALsizei DataPosFrac = converter->mFracOffset;
        ALuint64 DataSize64;
        ALsizei DstSize;
...
#define DECL_TEMPLATE(T)                                                       \
static void Mono2Stereo##T(ALfloat *dst, const T *src, ALsizei frames)\
{                                                                              \
    ALsizei i;                                                                 \
    for(i = 0;i < frames;i++)                                                  \
        dst[i*2 + 1] = dst[i*2 + 0] = Sample_##T(src[i]) * 0.707106781187f;    \
                                                                               ...
                                                                               \
static void Stereo2Mono##T(ALfloat *dst, const T *src, ALsizei frames)\
{                                                                              \
    ALsizei i;                                                                 \
    for(i = 0;i < frames;i++)                                                  \
